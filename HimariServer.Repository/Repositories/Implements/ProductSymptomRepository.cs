...
namespace HimariServer.Repository.Repositories.Implements
{
    public class ProductSymptomRepository : GenericRepository<ProductSymptom>, IProductSymptomRepository
    {
        ...
        public async Task<ProductSymptom?> FindByPartSymptomAndProduct(int partSymptomId, int productId)
        {
            return await _context.ProductSymptoms.FirstOrDefaultAsync(x => x.ProductId == productId && x.PartSymptomId == partSymptomId && !x.IsDeleted);
        }
    }
}
